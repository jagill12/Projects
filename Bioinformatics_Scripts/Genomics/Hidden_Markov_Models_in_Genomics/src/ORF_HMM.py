...
OUTPUT_DIR = BASE_DIR / "Output" / "ORF_HMM_Output"
...
def run_simulation(x, num_sequences, dinucleotide_matrix, model_name):
    ORF_lengths = []
    output_file = OUTPUT_DIR / f"John_Gill_ORF_{model_name}_Part_2.txt"
...
def plot_orf_distribution(ORF_lengths, model_name):
    ...
    plt.savefig(OUTPUT_DIR / f"{model_name}_ORF_distribution.png")
    plt.show()
...
def main():
    ...
    plt.legend()
    plt.savefig(OUTPUT_DIR / "Combined_ORF_distribution.png")
    plt.show()
